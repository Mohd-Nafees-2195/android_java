...
public class StudentDocsAdapterByExaminer extends FirebaseRecyclerAdapter<FileModel, StudentDocsAdapterByExaminer.MyViewHolder> {
     ...
     * @param options
     */
    public StudentDocsAdapterByExaminer(@NonNull FirebaseRecyclerOptions<FileModel> options) {
        super(options);
    }

    @Override
    protected void onBindViewHolder(@NonNull final StudentDocsAdapterByExaminer.MyViewHolder holder, @SuppressLint("RecyclerView") final int position, @NonNull final FileModel model)
    {
        ...
        if(model.getMessage().compareTo("Rejected By Examiner")==0){
        ...
        holder.verifyButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(model.visibility==1&&model.getMessage().compareTo("Verified By Guide")==0){
                    AlertDialog.Builder builder = new AlertDialog.Builder(holder.pdfName.getContext());
                    builder.setTitle("Verify Docs");
                    ...
                    builder.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            model.setMessage("Verified By Examiner");
                            FirebaseDatabase.getInstance().getReference("uploads").child(studentId).child(getRef(position).getKey())
                                    .setValue(model).addOnSuccessListener(new OnSuccessListener<Void>() {
                                        ...
                                        public void onSuccess(Void unused) {
                                            ...
                                            holder.verifyButton.setTextColor(android.graphics.Color.WHITE);
                                            String studentId=getRef(position).getParent().getKey();
                                            Intent intent=new Intent(holder.verifyButton.getContext(),FetchStudentDocsByExaminerActivity.class);
        ...
        holder.rejectButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(model.getVisibility()==1&&model.getMessage().compareTo("Verified By Guide")==0){
                    ...
                    builder.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            model.setMessage("Rejected By Examiner");
                            FirebaseDatabase.getInstance().getReference("uploads").child(studentId).child(getRef(position).getKey())
                                    .setValue(model).addOnSuccessListener(new OnSuccessListener<Void>() {
                                        @Override
                                        public void onSuccess(Void unused) {
                                            ...
                                            holder.rejectButton.setTextColor(android.graphics.Color.WHITE);
                                            String studentId=getRef(position).getParent().getKey();
                                            Intent intent=new Intent(holder.rejectButton.getContext(),FetchStudentDocsByExaminerActivity.class);
                                            intent.putExtra("studentId",studentId);
    ...
    @NonNull
    @Override
    public StudentDocsAdapterByExaminer.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
