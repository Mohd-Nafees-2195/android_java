...
public class StudentFetchDataActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_student_fetch_data);
        ...
        String roleTypeToFetchStuData=getIntent().getStringExtra("RoleTypeToFetchStuData");
        if(roleTypeToFetchStuData.compareTo("faculty")==0){
            FirebaseRecyclerOptions<StudentData> options =
                    new FirebaseRecyclerOptions.Builder<StudentData>()
                            .setQuery(FirebaseDatabase.getInstance().getReference().child("StudentDetails").orderByChild("guideEmail").equalTo(""), StudentData.class)
                            .build();

            myAdapter = new StudentAdapter(options);
